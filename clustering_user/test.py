...
CONFIG = {
    ...
    "MAX_CLUSTER_RADIUS_KM": 6,  # 클러스터 최대 반경 (이보다 먼 곳은 포함 안 함)
    "MIN_PLACES_PER_CATEGORY": 7,  # Seed 검증: 각 카테고리별 최소 장소 수
    "CLUSTER_DISTANCE_WEIGHT": 0.2  # 다음 Seed 선택 시 거리 가중치 (0~1)
}
...
def setup_logging(log_dir):
    ...
    # 로그 파일명: clustering_YYYYMMDD_HHMMSS.log
...
def greedy_clustering(df, n_clusters, travel_style, places_per_category, max_radius_km, min_places_per_category, cluster_distance_weight):
    ...
    for cluster_idx in range(n_clusters):
        ...
        if len(available_seeds) == 0:
            log_print(f"⚠️ 클러스터 {cluster_idx}: {seed_category}에서 사용 가능한 Seed 없음")
            # 다른 카테고리에서 시도
        ...
        for _, candidate in seed_candidates.iterrows():
            ...
            if is_valid:
                ...
                log_print(f"✅ 클러스터 {cluster_idx} Seed 검증 통과: {candidate['name']}")
        ...
        if seed_place is None:
            log_print(f"⚠️ 클러스터 {cluster_idx}: 검증을 통과한 Seed를 찾을 수 없음 (주변에 장소가 너무 적음)")
            # 검증 없이 가장 좋은 후보 사용
        ...
        if len(cluster_centers) > 0:
            ...
            log_print(f"🎯 클러스터 {cluster_idx} Seed: {seed_place['name']} ({seed_category}, Score: {seed_place['final_score']:.4f}, 최근접 클러스터: {min_dist:.1f}km)")
        else:
            log_print(f"🎯 클러스터 {cluster_idx} Seed: {seed_place['name']} ({seed_category}, Score: {seed_place['final_score']:.4f})")
...
def process_user(user_id, user_info, user_pref_dir, location_dict, output_dir):
    ...
    log_print(f"   여행 스타일: {travel_style}")
    log_print(f"{'='*70}")
    ...
    log_print(f"✅ Greedy 클러스터링 완료: {len(clusters)}개 클러스터")
