...
def select_best_accommodation(user_pref_file, clusters, location_dict, start_date, budget, duration_days, 
    ...
    """
    예산을 고려하여 최적의 숙소 선택 (가중 평균 사용)
    
    숙소 선택 로직:
    1. 클러스터 중심점의 가중 평균 계산
    2. 예산 계산: 전체 예산의 25%를 숙소에 할당
    3. 시즌/요일별 가격 적용
    4. 최종 점수 = 선호도(85%) + 거리(15%)
    """
    if len(clusters) == 0:
        return None
    ...
    log_print(f"🏨 숙소 선택 중...")
    log_print(f"{'='*70}")
    log_print(f"   단순 평균 중심: ({mean[0]:.4f}, {mean[1]:.4f})")
    
    dist = np.array([
        haversine((mean[0], mean[1]), (c['center_lat'], c['center_lng']), unit=Unit.KILOMETERS)
        for c in clusters
    ])
    
    log_print(f"   클러스터 간 거리: {', '.join([f'{d:.1f}km' for d in dist])}")
    
    weights = 1 / (1 + dist**2)
    weighted_center = np.average(coords, axis=0, weights=weights)
    overall_center_lat = weighted_center[0]
    overall_center_lng = weighted_center[1]
    
    log_print(f"   가중 평균 중심: ({overall_center_lat:.4f}, {overall_center_lng:.4f})")
    
    # ========================================
    # 예산 계산
    # ========================================
    accommodation_nights = duration_days - 1  # 숙박 일수
    total_accommodation_budget = (budget / 2) * 0.5  # 전체 예산의 25%
    per_night_budget = total_accommodation_budget / accommodation_nights  # 1박당 예산
    
    log_print(f"\n   💰 예산 정보:")
    log_print(f"      - 전체 예산: {budget:,}원")
    log_print(f"      - 숙소 총 예산 (25%): {total_accommodation_budget:,.0f}원")
    log_print(f"      - 숙박 일수: {accommodation_nights}박")
    log_print(f"      - 1박당 허용 금액: {per_night_budget:,.0f}원")
    ...
    log_print(f"   📅 시즌/요일 정보:")
    log_print(f"      - 시작 날짜: {start_date} ({start_weekday}요일)")
    log_print(f"      - 시즌: {season} ({'피크' if peak else '비피크'})")
    log_print(f"      - 요일: {day_type} ({'주말' if weekend else '평일'})")
    ...
    if len(accommodations) == 0:
        log_print(f"⚠️ 숙소 선호도 정보 없음")
        return None
    
    log_print(f"\n   숙소 후보: {len(accommodations)}개")
    ...
    for acc in accommodations:
        ...
        acc_lat = acc_info['latitude']
        acc_lng = acc_info['longitude']
        acc_name = acc_info['name']
        
        # 해당 시즌/요일의 가격
        acc_price = acc_info.get(price_column)
        
        # 가격 정보가 없거나 예산 초과하면 제외
        if pd.isna(acc_price) or acc_price is None:
            continue
        
        if acc_price > per_night_budget:
            continue
        
        distance = haversine(
            (overall_center_lat, overall_center_lng),
            (acc_lat, acc_lng),
            unit=Unit.KILOMETERS
        )
    ...
    if len(scored_accommodations) == 0:
        log_print(f"⚠️ 예산({per_night_budget:,.0f}원) 및 거리({max_distance_km}km) 조건을 만족하는 숙소 없음")
        return None
    
    log_print(f"   예산 내 후보: {len(scored_accommodations)}개")
    
    scored_accommodations.sort(key=lambda x: x['final_score'], reverse=True)
    best_accommodation = scored_accommodations[0]
    
    log_print(f"\n   ✅ 선택된 숙소: {best_accommodation['name']}")
    log_print(f"      - ID: {best_accommodation['id']}")
    log_print(f"      - 1박 가격: {best_accommodation['price']:,.0f}원 (예산: {per_night_budget:,.0f}원)")
    log_print(f"      - 선호도 점수: {best_accommodation['preference_score']:.4f}")
    log_print(f"      - 중심점 거리: {best_accommodation['distance']:.2f}km")
    log_print(f"      - 최종 점수: {best_accommodation['final_score']:.4f}")
    
    if len(scored_accommodations) > 1:
        log_print(f"\n   📋 상위 3개 후보:")
        for i, acc in enumerate(scored_accommodations[:3], 1):
            log_print(f"      {i}. {acc['name']} (점수: {acc['final_score']:.4f}, 거리: {acc['distance']:.2f}km, 가격: {acc['price']:,.0f}원)")
    
    return best_accommodation['id']
