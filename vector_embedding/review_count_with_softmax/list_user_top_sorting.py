...
CONFIG = {
    ...
    "OUTPUT_DIR": r"C:\Users\changjin\workspace\lab\pln\vector_embedding\review_count_with_softmax\for_cluster_list_sorting_user_results",
    "TOP_K": 150,
    "TOP_K_ACCOMMODATION": 20,  # 🔹 숙소는 20개만
    "GAMMA": 0.3   # 리뷰수 가중치
}
...
def attach_review_scores_and_final(results_by_cat, data_dir, gamma=0.3):
    ...
    for cat, scored_list in results_by_cat.items():
        ...
        # 🔹 상위 정렬 후 id만 추출
        sorted_ids = [
            item["id"] for item in sorted(cat_list, key=lambda x: x["final_score"], reverse=True)
        ]
        
        # 🔹 숙소는 상위 20개만 저장
        eng_cat = CATEGORY_TRANSLATE[cat]
        if eng_cat == "Accommodation":
            sorted_ids = sorted_ids[:20]
            print(f"   ⚠️ {eng_cat}: 상위 20개만 저장")

        # 🔹 카테고리 영어 이름 매핑 후 저장
        final_scores[eng_cat] = sorted_ids
...
for idx, user in user_df.iterrows():
    ...
    # ✅ JSON 한 줄로 저장
    out_path = os.path.join(CONFIG["OUTPUT_DIR"], f"{user_id}_recommendations_softmax.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(review_scores_by_cat, f, ensure_ascii=False, separators=(',', ':'))
